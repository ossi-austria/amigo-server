...
class APSessionsRepository<T : Session>(
    val repository: FindByIndexNameSessionRepository<T>
) : FindByIndexNameSessionRepository<T> by repository {

    companion object {
        private val log = LoggerFactory.getLogger(APSessionsRepository::class.java)
    ...
    override fun save(session: T) {
        if (repository is RedisIndexedSessionRepository) {
            ...
            repository.sessionRedisOperations.boundSetOps(principalRedisKey).add(session.id)
        } else {
