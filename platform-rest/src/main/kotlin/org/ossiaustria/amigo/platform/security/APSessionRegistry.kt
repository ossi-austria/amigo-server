...
@Component
class APSessionRegistry(
    @Qualifier("apSessionRepository") private val sessionRepository: FindByIndexNameSessionRepository<out Session>
) : SessionRegistry {
    companion object {
        private val log = LoggerFactory.getLogger(APSessionRegistry::class.java)
    }
    ...
    fun expireAllSessionsByUsername(userName: String): Int {
        val sessions = sessionRepository
            ...
            .map { APSessionInformation(it.value, sessionRepository) }
    ...
    override fun getAllSessions(principal: Any, includeExpiredSessions: Boolean): List<SessionInformation> {
        ...
        return sessions.map { APSessionInformation(it, sessionRepository) }
    }
    ...
    override fun getSessionInformation(sessionId: String?): SessionInformation? {
        ...
        return APSessionInformation(session, sessionRepository)
    }
